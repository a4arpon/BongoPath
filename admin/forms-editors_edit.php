...
if (!isset($_COOKIE['admin_cook'])) {
  ...
  if (!isset($_GET['post_id'])) {
    ...
    $sql_post_select = $con->query("SELECT * FROM `posts` Where `post_id`='$post_id'");
    $post = $sql_post_select->fetch_assoc();
  }
  // Functions
  function catStraper($con, $cat_name)
  {
    $cat_sql = $con->query("SELECT * FROM `menus_navbar` WHERE `name`='$cat_name'");
    $cat_link = $cat_sql->fetch_assoc();
    return $cat_link['link'];
  }
  function br_news_fetcher($con, $post_id)
  {
    $sql_br_news = $con->query("SELECT * FROM `breaking_news` Where `news_id`='$post_id'");
    if ($sql_br_news->num_rows > 0) {
      return "yes";
    } else {
      return "no";
    }
  ...
  function g_news_fetcher($con, $post_id)
  {
    $sql_br_news = $con->query("SELECT * FROM `google_news` Where `news_id`='$post_id'");
    if ($sql_br_news->num_rows > 0) {
      return "yes";
    } else {
      return "no";
...
<body>
  ...
  <main id="main" class="main">
    <section class="section">
      <form action="./lib/core/update_post.php?post_id=<?= $post['post_id'] ?>" method="post"
            ...
            <div class="card">
              <div class="card-body p-4">
                <div class="col-md-12 mb-2 px-2">
                  <h3>Post Title</h3>
                  <textarea name="post_title" class="w-100" rows="2"><?= $post['post_title'] ?></textarea>
                </div>
                <div class="d-md-flex">
                    ...
                    <div class="row gx-2">
                      <div class="col-6">
                        ...
                        <select name="post_cat" class="form-control">
                          <option value="<?= catStraper($con, $post['post_cat']) ?>">
                            <?= $post['post_cat'] ?>
                          </option>
                          ...
                          while ($nav_link = mysqli_fetch_assoc($qry_nav_menu)) {
                            if ($post['post_cat'] == $nav_link['name']) {
                              continue;
                            }
                          ?>
                      ...
                      <div class="col-6">
                        ...
                        <select name="br_news" class="form-control">
                          <?php
                          if (br_news_fetcher($con, $post_id) == "yes") {
                          ?>
                    ...
                    <div class="row gx-2">
                      <div class="col-md-2 mb-2 px-2">
                        ...
                        <select name="google_news" class="form-control mt-2">
                          <?php
                          if (g_news_fetcher($con, $post_id) == "yes") {
                          ?>
                          <option value="yes">Yes</option>
                          <option value="no">No</option>
                          <?php
                      ...
                      <div class="col-md-10 mb-2 px-2">
                        <h5>Image Alt Tag</h5>
                        <input type="text" name="f_image_alt" class="form-control" value="<?= $post['post_f_alt'] ?>">
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div class="col-lg-12 mb-2">
            <div class="card">
              <div class="card-body p-4">
                ...
                <textarea name="post_meta" class="form-control w-100 mb-2"
                  rows="10"><?= $post['post_meta'] ?></textarea>
                <div class="row gx-2 h-100 mt-2">
